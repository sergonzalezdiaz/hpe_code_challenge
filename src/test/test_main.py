...
def test_create_book():
    ...
    number_of_pages = 500
    author = "Test author"
    isbn = str(random.randint(1000000000009,9999999999999))
    number_of_copies = 7
    available_copies = 6
...
def test_reserve_book_incorrect():
    ...
    id = response_create.json()["id"]
    logger.info(f"New book created with id {id}")
    incorrect_id = id+1000
    logger.info(f"Trying to reserve 1 copy of book {incorrect_id}")
    response_reserve = client.post("/reserve", content=json.dumps([incorrect_id]))
    assert response_reserve.status_code == 400
